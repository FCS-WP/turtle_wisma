...
function handmade_shop_woocommerce_shortcodes($shortcodes) {
...
add_filter('cmsmasters_custom_shortcodes_filter', 'handmade_shop_woocommerce_shortcodes');
...
function cmsmasters_product_category($atts, $content = null) {
	...
	if (handmade_shop_locate_template($shortcode_path)) {
	...
	if ($query->have_posts()) : 
		...
		'>';
			...
			$out .= '<div class="woocommerce" category="' . $category . '" orderby="' . $orderby . '" order="' . $order . '">' . 
