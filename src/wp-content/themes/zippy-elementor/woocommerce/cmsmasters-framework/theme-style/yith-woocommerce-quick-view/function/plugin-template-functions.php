...
function handmade_shop_quick_view_button($product_id = 0, $label = '', $type = '', $use = '', $show = true) {
	global $woocommerce, 
		$product;
	...
	if (defined('YITH_WCQV_PREMIUM')) {
		...
		if (
			...
			add_action('wp_footer', 'handmade_shop_quick_view_modal_window');
		}
	...
	if ($quick_view_enable) {
		if ( 
			...
			if (defined('YITH_WCQV_PREMIUM')) {
				if ($position == 'image') {
					$out .= $button = '<div class="button cmsmasters_quick_view_button yith-wcqv-button cmsmasters-inside-thumb' . (($label) ? '' : ' empty cmsmasters_theme_icon_quick_view') . '"' . (($label) ? ' title="' . $label . '"' : '') . ' data-product_id="' . $product_id . '">' . $content . '</div>';
				} else {
...
function handmade_shop_quick_view_enqueue_styles() {
...
add_action('wp_enqueue_scripts', 'handmade_shop_quick_view_enqueue_styles');


function handmade_shop_quick_view_modal_window() {
	/* enqueue script */
