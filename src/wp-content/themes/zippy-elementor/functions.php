...
if (!function_exists('handmade_shop_system_fonts_list')) {
	function handmade_shop_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('handmade_shop_text_decoration_list')) {
	function handmade_shop_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'handmade-shop'), 
			'underline' => esc_html__('underline', 'handmade-shop'), 
			'overline' => esc_html__('overline', 'handmade-shop'), 
			'line-through' => esc_html__('line-through', 'handmade-shop'), 
		);
...
if (!function_exists('handmade_shop_custom_color_schemes_list')) {
	function handmade_shop_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'handmade-shop'), 
			'second' => esc_html__('Custom 2', 'handmade-shop'), 
			'third' => esc_html__('Custom 3', 'handmade-shop') 
		);
		
		
		return apply_filters('handmade_shop_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('handmade_shop_locate_template')) {
	function handmade_shop_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_handmade-shop_theme_style') ? get_option('cmsmasters_handmade-shop_theme_style') : ''));
...
if (!function_exists('handmade_shop_all_theme_styles')) {
	function handmade_shop_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('handmade_shop_all_color_schemes_list')) {
	function handmade_shop_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'handmade-shop'), 
			'header' => 		esc_html__('Header', 'handmade-shop'), 
			'navigation' => 	esc_html__('Navigation', 'handmade-shop'), 
			'header_top' => 	esc_html__('Header Top', 'handmade-shop'), 
			'footer' => 		esc_html__('Footer', 'handmade-shop') 
		);
...
if (!function_exists('handmade_shop_load_theme_textdomain')) {
	function handmade_shop_load_theme_textdomain() {
		load_theme_textdomain('handmade-shop', CMSMASTERS_VARS . '/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'handmade_shop_load_theme_textdomain')) {
	add_action('after_setup_theme', 'handmade_shop_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('handmade_shop_theme_activation')) {
	function handmade_shop_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'handmade-shop') {
			add_option('cmsmasters_active_theme', 'handmade-shop', '', 'yes');
...
add_action('after_switch_theme', 'handmade_shop_theme_activation');



// Framework Deactivation
if (!function_exists('handmade_shop_theme_deactivation')) {
	function handmade_shop_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'handmade_shop_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('handmade_shop_plugin_activation')) {
	function handmade_shop_plugin_activation($plugin, $network_activation) {
...
add_action('activated_plugin', 'handmade_shop_plugin_activation', 10, 2);


if (!function_exists('handmade_shop_plugin_activation_regenerate')) {
	function handmade_shop_plugin_activation_regenerate() {
...
add_action('init', 'handmade_shop_plugin_activation_regenerate');



/* Yith WooCommerce Ajax Search Color Default */
function handmade_shop_woocommerce_catalog_images_default() {
...
add_action('activate_woocommerce/woocommerce.php', 'handmade_shop_woocommerce_catalog_images_default');



/* Yith WooCommerce Ajax Search Color Default */
function handmade_shop_ajax_search_premium_colors_default() {
...
add_action('activate_yith-woocommerce-ajax-search-premium/init.php', 'handmade_shop_ajax_search_premium_colors_default');



/* Yith WooCommerce Ajax Search Color Default */
function handmade_shop_ajax_search_colors_default() {
	update_option('yith_wcas_search_submit_label', '');
}

add_action('activate_yith-woocommerce-ajax-search/init.php', 'handmade_shop_ajax_search_colors_default');



/* Yith WooCommerce Quick View Color Default */
function handmade_shop_quick_view_colors_default() {
...
add_action('activate_yith-woocommerce-quick-view/init.php', 'handmade_shop_quick_view_colors_default');



/* Yith WooCommerce Quick View Color Default */
function handmade_shop_quick_view_premium_colors_default() {
	update_option('yith-wcqv-background-modal', '');
...
add_action('activate_yith-woocommerce-quick-view-premium/init.php', 'handmade_shop_quick_view_premium_colors_default');



function handmade_shop_run_reinit_import_options($post_id, $key, $value) {
	if (!get_post_meta($post_id, 'cmsmasters_heading', true)) {
		$custom_post_meta_fields = handmade_shop_get_custom_all_meta_fields();
...
add_action('import_post_meta', 'handmade_shop_run_reinit_import_options', 10, 3);
