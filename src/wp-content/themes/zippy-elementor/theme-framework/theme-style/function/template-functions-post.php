...
function handmade_shop_post_heading($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function handmade_shop_post_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function handmade_shop_get_post_date($template_type = 'page', $layout_type = 'default', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['handmade-shop' . '_blog_post_date']) {
			$out .= '<span class="cmsmasters_post_date">' . 
				esc_html__('on', 'handmade-shop') . ' ' . 
				'<abbr class="published" title="' . esc_attr(get_the_date()) . '">' . 
					esc_html(get_the_date()) . 
...
function handmade_shop_get_post_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_post_author">' . 
			esc_html__('by', 'handmade-shop') . ' ' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'handmade-shop') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author">' . 
		...
		if ($cmsmasters_option['handmade-shop' . '_blog_post_author']) {
			$out .= '<span class="cmsmasters_post_author">' . 
				esc_html__('by', 'handmade-shop') . ' ' . 
				'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'handmade-shop') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author">' . 
					'<span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span>' . 
...
function handmade_shop_get_post_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['handmade-shop' . '_blog_post_cat']) {
				$out .= '<span class="cmsmasters_post_category">' . 
					handmade_shop_get_the_category_list($cmsmasters_id, $taxonomy, '') . 
				'</span>';
			}
...
function handmade_shop_get_post_tags($show = true) {
	if (get_the_tags()) {
		$cmsmasters_option = handmade_shop_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['handmade-shop' . '_blog_post_tag']) {
...
function handmade_shop_post_exc_cont($length = 20, $show = true) {
	$out = cmsmasters_divpdel('<div class="cmsmasters_post_content entry-content">' . "\n" . 
		wpautop(handmade_shop_excerpt($length, false)) . 
	'</div>' . "\n");
	
	
	if (handmade_shop_excerpt($length, false) != '') {
		if ($show) {
...
function handmade_shop_get_post_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['handmade-shop' . '_blog_post_like']) {
			$out = cmsmasters_like('cmsmasters_post_likes');
		}
	}
...
function handmade_shop_get_post_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = handmade_shop_get_comments('cmsmasters_post_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = handmade_shop_get_global_options();
			
			if ($cmsmasters_option['handmade-shop' . '_blog_post_comment']) {
				$out = handmade_shop_get_comments('cmsmasters_post_comments');
			}
		}
	}
...
function handmade_shop_post_more($cmsmasters_id, $show = true) {
	$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
	
	
	if ($cmsmasters_post_read_more == '') {
		$cmsmasters_post_read_more = esc_html__('Read More', 'handmade-shop');
	}
...
function handmade_shop_post_format_icon_placeholder($cmsmasters_id, $format = 'std', $show = true) {
	$out = '<figure>' . 
